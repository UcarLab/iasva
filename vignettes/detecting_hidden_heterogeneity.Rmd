...
## Install packages for the firt use
...
Here, using the find.markers() function we find marker genes (n=105 genes) that are significantly associated with SV2 (multiple testing adjusted p-value < 0.05, default significance cutoff, and a high R-squared value (R-squared > 0.3, default R-squared cutoff).  
...
As shown above, tSNE fails to detect the outlier cells that are identified by IA-SVA. Same color coding is used as above. 
