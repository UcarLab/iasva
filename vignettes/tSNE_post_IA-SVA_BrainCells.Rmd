...
tsne.res <- Rtsne(iasva.sv, dims = 2)
plot(tsne.res$Y, main="tSNE", pch=21, bg=color.vec[Cell_Type], oma=c(4,4,6,12))
legend("topright", levels(Cell_Type), fill=color.vec, bty="n")
...
Here, we apply tSNE on the marker genes for SV2 obtained from IA-SVA
```{r run_pca, echo=TRUE, fig.width=8, fig.height=8}
pairs(pca.res[,1:10], main="PCA", pch=21, bg=color.vec[Cell_Type], oma=c(4,4,6,12))
legend("right", levels(Cell_Type), fill=color.vec, bty="n")
```

```{r spectral_tSNE, echo=TRUE, fig.width=8, fig.height=8}
set.seed(3455563)
tsne.res <- Rtsne(pca.res[,c(1:10)], dims = 2)
plot(tsne.res$Y, main="tSNE", pch=21, bg=color.vec[Cell_Type], oma=c(4,4,6,12))
