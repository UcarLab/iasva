---
title: "IA-SVA based feature selection improves the performance of clustering algorithms (e.g., tSNE)"
...
## Extract three cell types (GCG, INS, KRT19 expressed cells) from non-diabetics and filter out low-expressed genes 
...
```
As shown in the above figure, SV1, SV2 and SV4 separate cells.


## Find marker genes for SV1, SV2 and SV4.
Here, we find marker genes significantly associated with SV1, SV2 and SV4 (multiple testing adjusted p-value < 0.05, default significance cutoff) and having a high R-squared value (R-squared > 0.3) using the find.markers() function.
...
## Run tSNE post IA-SVA, i.e., run tSNE on maker genes for SV1, SV2 and SV4 obtained from IA-SVA.
Here, we apply tSNE on the marker genes for SV1, SV2 and SV4 obtained from IA-SVA
